...
def avgpool(input_tensor, pool_size=128): # BSND in,  BSND out
...
def rearrange_with_remaining(tensor, latent_shape_q, latent_shape_k): # BSND in ,  BSND out
    '''
    ...
    if (hq % 8 != 0) or (wq % 8 != 0):
        ...
        tensor_hwt = rearrange(tensor_hwt, 'b f (hn hb) (wn wb) n d -> b f (hn wn hb wb) n d', f=frame_num - 1, hb=8,
                               wb=8, hn=hq // 8, wn=wq // 8)
        if hq % 8 != 0:
...
def inv_rearrange_with_remaining(tensor, latent_shape_q, latent_shape_k): # BSND in ,  BSND out
    b, s, n, d = tensor.shape
    ...
    if (hq % 8 != 0) or (hq % 8 != 0):
        ...
        tensor_hwt = rearrange(tensor_hwt, 'b f (hn wn hb wb) n d -> b f (hn hb) (wn wb) n d', f=frame_num - 1, hb=8,
                               wb=8, hn=hq // 8, wn=wq // 8)
        if wq % 8 != 0:
...
def do_tensor_rearrange_pooling(query, key, value, text_len, pool_size, latent_shape_q, latent_shape_k):
    '''
    张量的分块重排 + 池化操作
    '''
    ...
    if text_len != 0:
        ...
        tensor_i_2 = rearrange_with_remaining(tensor_i, latent_shape_q, latent_shape_k)
        tensor_i_pool = avgpool(tensor_i_2, pool_size)
        tensor_t_pool = avgpool(tensor_t, pool_size)
        tensor = torch.concat((tensor_i_2, tensor_t), dim=1)
        tensor_pool = torch.concat((tensor_i_pool, tensor_t_pool), dim=1)
    else:
        b, s, n, d = tensor.shape
        tensor = rearrange_with_remaining(tensor, latent_shape_q, latent_shape_k)
        tensor_pool = avgpool(tensor, pool_size)
    query_, key_, value_ = torch.chunk(tensor, 3, dim=0)
    return query_, key_, value_, tensor_pool


def do_tensor_inv_rearrange(tensor, text_len, latent_shape_q, latent_shape_k):
    if text_len != 0:
        tensor_t = tensor[:, -text_len:, :, :]
        tensor_i = tensor[:, :-text_len, :, :]

        tensor_i = inv_rearrange_with_remaining(tensor_i, latent_shape_q, latent_shape_k)
        tensor = torch.concat((tensor_t, tensor_i), dim=1)
    else:
        tensor = inv_rearrange_with_remaining(tensor, latent_shape_q, latent_shape_k)
    return tensor
